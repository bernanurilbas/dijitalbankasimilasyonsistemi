...
const AuthLayout = () => {
  ...
  return (
    <div className={`min-h-screen w-full flex items-center justify-center p-4 sm:p-6 md:p-8 font-sans relative overflow-hidden transition-colors duration-300 ${
      theme === 'dark' 
      ...
      <div className={`absolute top-[-15%] left-[-10%] w-[600px] h-[600px] rounded-full blur-[130px] pointer-events-none z-0 transition-colors duration-300 ${
        theme === 'dark' ? 'bg-blue-500/25' : 'bg-blue-300/30'
      }`} />
      <div className={`absolute top-[-30%] left-[15%] w-[650px] h-[650px] rounded-full blur-[140px] pointer-events-none z-0 transition-colors duration-300 ${
        theme === 'dark' ? 'bg-blue-600/20' : 'bg-blue-400/20'
      }`} />
      <div className={`absolute bottom-[-10%] right-[-10%] w-[600px] h-[600px] rounded-full blur-[140px] pointer-events-none z-0 transition-colors duration-300 ${
        theme === 'dark' ? 'bg-[#14b8a6]/10' : 'bg-teal-300/20'
      }`} />
      <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] rounded-full blur-[150px] pointer-events-none z-0 transition-colors duration-300 ${
        theme === 'dark' ? 'bg-blue-600/12' : 'bg-blue-400/10'
      }`} />
      ...
      */}
      <div 
        className={`w-full max-w-5xl rounded-[32px] overflow-hidden shadow-2xl flex flex-col md:flex-row min-h-[620px] relative bg-cover bg-center z-10 transition-all duration-300 ${
          theme === 'dark' 
      ...
      >
        ...
        <div className="relative w-full md:w-[55%] flex flex-col justify-between p-10 min-h-[300px] md:min-h-auto shrink-0 z-10">
          ...
          {/* Action link block (Here you are: sign in block) */}
          <div className="relative z-10 mt-auto">
            {isLoginPage ? (
              <div className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl backdrop-blur text-xs font-bold transition-all ${
                theme === 'dark'
        ...
        */}
        <div className={`w-full md:w-[45%] flex items-center justify-center p-6 sm:p-10 backdrop-blur-2xl transition-all duration-300 relative z-10 ${
          theme === 'dark' 
